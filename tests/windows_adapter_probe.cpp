...
main(int argc, char** argv)
{
    ...
    if (!WaitForRevision(controller, sink, 2))
    {
        controller.Stop();
        std::cerr << "delta ns-3 metrics timed out\n";
        return 8;
    }
    controller.Stop();

    if (sink.states[{2, "WEAPON-1"}] != EffectState::Blocked ||
        sink.states[{2, "RADAR-1"}] != EffectState::Degraded)
    {
        std::cerr << "network effects did not reach the AFSIM sink\n";
        return 9;
    }

    std::cout << "revision=2 weapon=BLOCKED radar=DEGRADED\n";
    return 0;
}
